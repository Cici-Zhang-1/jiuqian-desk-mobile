  ...
  <div class="col-12">
    <div class="card border-0 mb-3" v-if="!error && !loading && card.data && card.data.num">
      <div class="card-body p-0 card-body-h">
        ...
        <scan-pack-board-table :table="card.data.content" :tableThead="card.elements" :qrcode="qrcode" :showAll="showAll" :thick="thick"/>
...
export default {
  ...
  props: {
    ...
    showAll: { // 显示内容 2：显示正在扫描，1：显示全部，0：显示未扫描
      type: Number,
      default: 2
    },
    refresh: {
  ...
  computed: {
    ...
    packNum: {
      get () {
        return (this.card.data && this.card.data.num && parseInt(this.card.data['order_product']['pack'])) || 0
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    setPacked () {
      return parseInt(this.scanning.pack) > 0
    },
    disposeSure (e) {
      this.setQrcode()
    },
