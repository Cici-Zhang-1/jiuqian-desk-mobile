<template>
  <tr :class="{'table-danger': trData.checked, 'd-none': isShow()}" @click="activeTr($event.currentTarget)" @dblclick="multipleActiveTr" v-if="thickFilter()">
...
export default {
  ...
  methods: {
    isShow () {
      if (this.showAll === 2) { // 正在扫描
        return !this.trData.checked
      } else if (this.showAll === 1) { // 显示全部
        return false
      } else { // 显示未扫描
        return !(parseInt(this.trData['pack']) === 0 && !this.trData.checked)
      }
    },
    activeTr (e) { // 单行激活
