  ...
  <div class="tab-pane fade" id="dismantleF" role="tabpanel" aria-labelledby="door">
    ...
    <table class="table-center table-form table table-bordered table-responsive text-nowrap" id="dismantleFTable" v-if="!loading && activeOrderProduct">
      ...
      <tr v-for="(item, key, index) in activeOrderProduct['order_product_board_plate']" :key="index">
        ...
        <td is="dismantle-server" :server="item['server']" v-model="item['server']" @focusout-server="changeServer($event)"></td>
...
export default {
  ...
  methods: {
    ...
    copy () { // 复制一行内容
      let self = this
      $('#dismantleFTable tbody tr input[name="copy"]').off('change.copy').on('change.copy', function (i, v) {
        let copy = $(this).val()
        if (copy > 0) {
    ...
    addLine () { // 新建一行
      let self = this
      $('#dismantleFTable tbody tr:last').off('click.addLine').on('click.addLine', function (i, v) {
        self.activeOrderProduct['order_product_board_plate'].push(cloneData(self.demoData))
        self.maxNum = self.activeOrderProduct['order_product_board_plate'].length + 1
      })
    },
    highlightTr () { // 高亮选中一行
      $('#dismantleFTable').find('tbody tr').each(function (i, v) {
