<template>
  <div class="tab-pane fade" id="postSaleF" role="tabpanel" aria-labelledby="server">
    ...
    <div class="row">
      <div class="col-9">
        <table class="table-center table-form table table-bordered table-responsive text-nowrap" id="postSaleFTable" v-if="!loading && activeOrderProduct">
          <thead>
          <tr>
            <th class="td-xs" >#</th>
...
export default {
  ...
  computed: {
    orderProduct: {
      get () {
        return this.$store.getters.getDismantleData({ uri: this.postSaleUrl, child: 'F' })
      },
      set (Value) {
      }
    },
    ...
    activeOrderProduct: {
      get () {
        return this.$store.getters.getActiveDismantleData({ uri: this.postSaleUrl, child: 'F' })
      }
    },
    serverData: {
      get () {
  ...
  mounted () {
    this.highlightTr('postSaleFTable')
    this.addLine('postSaleFTable')
    this.handleDirection('postSaleFTable')
    if (this.amountFocus) {
      this.amountFocus = false
      $('#postSaleFTable').find('tbody tr').eq(this.activeOrderProduct['order_product_board_plate'].length - 1).find('input[name="amount"]').focus()
    }
  ...
  methods: {
    ...
    changeServer (e) {
      ...
      Data['unit_price'] = serverData['saler_unit_price']
