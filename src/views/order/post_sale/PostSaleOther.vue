<template>
  <div class="tab-pane fade" id="postSaleG" role="tabpanel" aria-labelledby="other">
    ...
    <div class="row">
      <div class="col-9">
        <table class="table-center table-form table table-bordered table-responsive text-nowrap" id="postSaleGTable" v-if="!loading && activeOrderProduct">
          <thead>
          <tr>
            <th class="td-xs" >#</th>
...
export default {
  ...
  computed: {
    orderProduct: {
      get () {
        return this.$store.getters.getDismantleData({ uri: this.postSaleUrl, child: 'G' })
      },
      set (Value) {
      }
    },
    ...
    activeOrderProduct: {
      get () {
        return this.$store.getters.getActiveDismantleData({ uri: this.postSaleUrl, child: 'G' })
      }
    },
    otherData: {
      get () {
  ...
  mounted () {
    this.highlightTr('postSaleGTable')
    this.addLine('postSaleGTable')
    this.handleDirection('postSaleGTable')
  ...
  updated () {
    this.highlightTr('postSaleGTable')
    this.addLine('postSaleGTable')
    this.copy('postSaleGTable')
    this.handleDirection('postSaleGTable')
    if (this.amountFocus) {
      this.amountFocus = false
      $('#postSaleGTable').find('tbody tr').eq(this.activeOrderProduct['order_product_board_plate'].length - 1).find('input[name="amount"]').focus()
    }
  },
  methods: {
    ...
    changeOther (e) {
      ...
      Data['unit_price'] = otherData.saler_unit_price
