<template>
  <tr :class="{'table-success': trData.checked, 'table-danger': unPay()}" :id="id" @click="activeTr($event.currentTarget)" @dblclick="multipleActiveTr" v-if="quickSearch()">
...
export default {
  ...
  methods: {
    unPay () {
      if ((this.trData['sum'] - this.trData['payed']) > this.trData['balance']) {
        if (this.trData['pay_status'] !== 1 && this.trData['pay_status'] !== '1') {
          if (this.trData['payterms'] === '宽松发货') {
            if (this.trData['application_status'] !== 2 && this.trData['application_status'] !== '2') {
              return true
            }
          } else {
            return true
