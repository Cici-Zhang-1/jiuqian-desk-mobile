<template>
  <table class="table table-striped table-responsive j-text-nowrap j-table" :id="id">
    ...
    <tbody >
      <tr @click="inactiveTr" is="wait-delivery-tr" v-for="(value, key, index) in table" :trData="value" :tableThead="tableThead" :key="index" :search="search"></tr>
    </tbody>
  </table>
</template>

<script>
import WaitDeliveryTr from './WaitDeliveryTr'
import { nameToId } from '@/assets/js/custom'

export default {
  name: 'WaitDeliveryTable',
  props: {
    table: {
      type: Object | Array,
      required: true
